...
/**
 * Helper methods related to requesting and receiving EarthquakeContainer data from USGS.
 */
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link EarthquakeContainer} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthquakeContainer> extractEarthquakeContainers() {

        // Create an empty ArrayList that we can start adding EarthquakeContainers to
        ArrayList<EarthquakeContainer> EarthquakeContainers = new ArrayList<>();
        ...
        try {
          ...
          JSONObject jso=new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray jarr=jso.getJSONArray("features");

            for(int i=0;i<jarr.length();i++){

                JSONObject j1=jarr.getJSONObject(i).getJSONObject("properties");

                double mag=j1.getDouble("mag");
                String city=j1.getString("place");
               int time= j1.getInt("time");
                Date d=new Date(time);
                String url=j1.getString("url");

                EarthquakeContainers.add(new EarthquakeContainer(mag,city,d,url));
            }
            // build up a list of EarthquakeContainer objects with the corresponding data.

        } catch (JSONException e) {
            // If an error is thrown when executing
            // any of the above statements in the "try" block,
            // catch the exception here, so the app doesn't crash. Print a log message
            // with the message from the exception.
            Log.e("QueryUtils", "Problem parsing the EarthquakeContainer JSON results", e);
        }

        // Return the list of EarthquakeContainers
        return EarthquakeContainers;
    }
